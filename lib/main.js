...
pluginPanel.port.on('install-app', function () {
  ...
  if (ret == Ci.nsIFilePicker.returnOK || ret == Ci.nsIFilePicker.returnReplace) {
    ...
    webappsJsonContent = webappsJsonContent.substring(0, webappsJsonContent.length-1) +
      ', "' + newAppId + '" : '  + JSON.stringify(newAppContent) + '}';

    //console.log('New webapp content is:', newAppContent)
    var stream = ioFile.open(webappsFilePath, 'w');
    stream.write(webappsJsonContent);
    stream.close();
  }
  pluginPanel.hide();
});
