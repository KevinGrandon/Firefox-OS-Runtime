...
pluginPanel.port.on('install-app', function () {
  ...
  if (ret == Ci.nsIFilePicker.returnOK || ret == Ci.nsIFilePicker.returnReplace) {
    ...
    let manifest = ioFile.read(manifestFile);
    ...
    let appShortName = Date.now();
    let newAppId = appShortName + '.gaiamobile.org';
    let app = {
      receipt: null,
    ...
    let profileAppDir = profileDir + '/webapps/' + newAppId + '/';
    ioFile.mkpath(profileAppDir);
    profileAppDir = ioFile.open(profileAppDir + 'manifest.webapp', 'w');

    profileAppDir.write(JSON.stringify(manifest));
    profileAppDir.close();
    ...
    reg._saveApps(function() {
      ...
      // Save the app to our gaia build preferences
      let prefs = Cc['@mozilla.org/preferences-service;1']
        .getService(Ci.nsIPrefBranch);
      let prefString = prefs.getComplexValue(
        'extensions.gaia.app_relative_path',
        Ci.nsISupportsString
      );

      let prefSupportString = Cc["@mozilla.org/supports-string;1"]
        .createInstance(Ci.nsISupportsString);
      prefSupportString.data = prefString + ' test_apps/' + appShortName;
      ...
      // Set the preference for localDomains
      let localDomainPrefString = prefs.getComplexValue(
        'network.dns.localDomains',
        Ci.nsISupportsString
      );

      let newLocalDomainString = Cc["@mozilla.org/supports-string;1"]
        .createInstance(Ci.nsISupportsString);
      newLocalDomainString.data = localDomainPrefString + ',' + appShortName + '.gaiamobile.org';

      prefs.setComplexValue(
        'network.dns.localDomains',
        Ci.nsISupportsString,
        newLocalDomainString
      );

      // Need to save the preferences after setting them
      /*
